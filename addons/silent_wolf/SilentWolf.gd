...
func send_get_request(http_node: HTTPRequest, request_url: String):
	...
	# Esperar a que el HTTPRequest esté realmente en el árbol (call_deferred
	# de add_child no garantiza que esté listo tras una sola espera fija).
	var _intentos = 0
	while !http_node.is_inside_tree() and _intentos < 30:
...
func send_post_request(http_node, request_url, payload):
	...
	for path in paths_with_values_to_hash:
		...
		if check_string_in_url(path, request_url):
			...
			break
	var use_ssl = true
	# Esperar a que el HTTPRequest esté en el árbol (la espera fija original
	# no es suficiente en todos los casos y disparaba ERR_UNCONFIGURED).
	var _intentos = 0
	while !http_node.is_inside_tree() and _intentos < 30:
		await get_tree().process_frame
		_intentos += 1
